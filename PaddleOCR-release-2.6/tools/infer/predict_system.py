    ...
    for idx, image_file in enumerate(image_file_list):
        ...
        for index, img in enumerate(imgs):
            ...
            if is_visualize:
                ...
                txt_save_dir = '../../Save_txt'
                s = os.path.basename(image_file)
                img_name = os.path.splitext(s)[0]
                with open(
                        os.path.join(txt_save_dir + "/" + img_name + "_system_results.txt"),
                        'w',
                        encoding='utf-8'
                ) as f:
                    # 问题就是不能直接创建一个TXT格式的文件！
                    # 其实是因为没有创建对应的二级文件夹！！明白了！
                    for i in range(len(txts)-1):
                        f.writelines(txts[i])
                        f.writelines('\n')
                    logger.debug("The visualized txt saved in {}".format(
                        os.path.join(txt_save_dir + '/' + img_name + "_system_results.txt")))
